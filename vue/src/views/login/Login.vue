...
function goHome() {
    ...
    myForm.value.validate((valid) => {
        if (valid) {
            // 表单验证通过
            ElMessage({ message: '表单校验通过', type: 'success' })
            // 校验验证码
            // 将前端输入的验证码发送给后端
            request.post('/captcha/verify', { code: user.value.captcha }).then(response => {
                // console.log(response);
                // 如果验证码校验通过了，才往下走
                if (response.code == 200) {
                    ElMessage({ message: '验证码正确', type: 'success' })
                    // 登录
                    // post的参数除了可以直接传user.value，
                    // 还可以这样传：{"name": user.value.name, "password": user.value.password}
                    request.post('/user-login', { "name": user.value.name, "password": user.value.password }).then(response => {
                        ...
                        } else { ElMessage({ message: '用户名或密码错误', type: 'error' }) }
                    }).catch(err => {
                        console.error(err);
                        ElMessage({ message: '系统异常错误', type: 'error' })
                    })
                } else { ElMessage({ message: '验证码不通过', type: 'error' }) }
            }).catch(err => { console.error(err); })
        } else { ElMessage({ message: '表单校验失败', type: 'error' }) }
    });
}
