    ...
    <el-dialog v-model="dialogVisible" title="注册成功" width="400" :before-close="handleClose">
        ...
        <template #footer>
            <div class="dialog-footer">
                <el-button type="primarykey" @click="copyText">复制到剪切板</el-button>
            </div>
        </template>
    </el-dialog>
</template>
...
const user = reactive({
    ...
    password: '',
    password2: '',
    gender: computed(() => { return gender.value ? '男' : '女'; }),
    phone: '',
    address: '',
    birthday: computed(() => { return birthday.value ? birthday.value.toISOString() : "" })
})
...
const { validName, validPassword, validPassword2, validGender, validPhone, validAddress, validBirthday } = useValid(errWord)
